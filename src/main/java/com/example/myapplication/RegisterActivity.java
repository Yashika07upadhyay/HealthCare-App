...
public class RegisterActivity extends AppCompatActivity {
    EditText edUsername,edConfirm,edPassword,edEmail;
    TextView tw;
    Button btn;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        edConfirm=findViewById(R.id.editTextRegFees);
        edPassword=findViewById(R.id.editTextRegContact);
        edEmail=findViewById(R.id.editTextRegAddress);
        btn = findViewById(R.id.buttonBookAppointment);
        tw = findViewById(R.id.textViewRegUser);
                ...
                if(username.length()==0||password.length()==0||confirm.length()==0||email.length()==0){
                    ...
                    if(password.compareTo(confirm)==0){
                     if(isValid(password)){
                         ...
                         Toast.makeText(getApplicationContext(),"Password must contain atleast 8 characters,letters,digits and special symbol",Toast.LENGTH_SHORT).show();
                     }
                    }
                    else{
